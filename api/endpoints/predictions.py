...
def get_predictions_on_fixture(fixture_id: int, db: Session = Depends(get_db)):
...
def get_predictions_in_league(league_id: int,user_id:int, db: Session = Depends(get_db)):
...
def update_prediction(prediction_id:int,scores:PredictionUpdate ,user=Depends(get_current_user),db=Depends(get_db)):
    return crud_predictions.update_user_prediction(user_id=user.id,prediction_id=prediction_id,score=scores,db=db)

@router.delete('/delete')
def delete_prediction(prediction_id:int,user=Depends(get_current_user),db=Depends(get_db)):
    return crud_predictions.delete_user_prediction(user_id=user.id,prediction_id=prediction_id,db=db)
