...
from schemas.prediction import PredictionCreate, PredictionSchema,PredictionUpdate
...
@router.get("/user", response_model=List[PredictionSchema])
...
@router.get("/fixture", response_model=List[PredictionSchema])
...
@router.get("/user/league", response_model=List[PredictionSchema])
async def get_predictions_in_league(league: str,user_id:int, db: Session = Depends(get_db)):
    return await crud_predictions.get_user_predictions_in_league(db=db, league=league,user_id=user_id)

@router.put('/update',response_model=PredictionSchema)
